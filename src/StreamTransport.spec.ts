...
async function transferWire(wire: unknown): Promise<unknown> {
...
describe("native stream cleanup", () => {
  it("ignores queued chunks and close messages after the consumer cancels", async () => {
    for (let attempt = 0; attempt < 10; attempt += 1) {
      ...
      const reader = readStreamWire<string>(await transferWire(wire)).getReader();
  ...
  it.each(["resolve", "reject", "pending"])(
    ...
    async (cleanup) => {
      ...
      await expect(readStreamWire(await transferWire(wire)).getReader().read()).rejects.toMatchObject({
        name: "DataCloneError",
      });
      await nextTask();
  ...
  it(
    "discards an unused stream response",
    async () => {
  ...
  it(
    "stops pulling when the consumer pauses",
    async () => {
      let pulled = 0;
      let resolveCancelled!: () => void;
      const cancellation = new Promise<void>((resolve) => {
