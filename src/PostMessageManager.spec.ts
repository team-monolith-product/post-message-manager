...
describe("stream transport", () => {
  ...
  it.each([false, true])(
    "does not discard another manager's stream response (native: %s)",
    async (native) => {
      const consumer = new PostMessageManagerImpl();
      const messageType = `stream:other-manager:${native}`;
      manager.register({
        messageType,
        callback: () =>
  ...
  it.each([[false, false], [false, true], [true, false], [true, true]])(
    "cancels a late response after the opening timeout (native: %s, aborted: %s)",
    async (native, aborted) => {
      const cancelled = jest.fn<(reason?: unknown) => void>();
      let resolveSource!: (source: ReadableStream<string>) => void;
      const messageType = `stream:late-timeout:${native}:${aborted}`;
      manager.register({
        messageType,
        callback: async () =>
          createStreamWire(
